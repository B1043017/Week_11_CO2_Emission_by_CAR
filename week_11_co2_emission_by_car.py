...
Original file is located at
    https://colab.research.google.com/drive/1AzbdAMUMGR7I_O6Ae-xrrIX7QtexCAjm
...
from sklearn.metrics import r2_score, mean_squared_error,mean_absolute_error #absolute 取絕對值
...
---
Data: $(x^i,y^i)$'s pair
$$ \hat y= A x+B\approx 39\text[Enginesize]$$
"""
...
def LinearＭodel(X,y,features_,split_size=0.2):
  X_train,X_test,y_train,y_test = train_test_split(X,y,test_size=split_size,random_state=42)
  reg_model=linear_model.LinearRegression()
  reg_model.fit(X_train,y_train)
  #exbiting the coeficient A and B that the model generated
  #print (f'(B)intercept:{reg_model.intercept_}|(A)inclination:{reg_model.coef_}')
  #print("Estimated Linear Model\n[CO2]=")
  #Get the cofficients and intercept
  ...
  #Showing metrics to check the acuracy of our model
  print(f'Sum of squared error(SSE):{np.sum((predict_-y_test)**2)}')
  print(f'Mean squred error(MSE):{mean_squared_error(y_test,predict_)}')
  print(f'Mean absoluted error(MSE):{mean_squared_error(y_test,predict_)}')
  print(f'Sqrt of mean squred error(RMSE):{sqrt(mean_squared_error(y_test,predict_))}')
  print(f'R2-score:{r2_score(predict_,y_test)}')
...
fig.add_trace(go.Scatter3d(x=X['ENGINESIZE'],y=X['CYLINDERS'],z=y,mode='markers',marker=dict(color='orchid')))
